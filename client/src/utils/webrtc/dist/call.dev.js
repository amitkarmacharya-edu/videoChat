...
var Call = function Call(params) {
  ...
  this.startTime = null; // Public callbacks. Keep it sorted.
...
Call.prototype.hangup = function (async) {
  ...
  if (async) {
    var errorHandler = function errorHandler(errorString, error) {
  ...
  var executeStep = function executeStep(executor, errorString) {
    try {
      executor();
    } catch (ex) {
...
Call.prototype.onRemoteHangup = function () {
  this.startTime = null; // On remote hangup this client becomes the new initiator.
...
Call.prototype.connectToRoom_ = function (roomId) {
  this.params_.roomId = roomId; // Asynchronously open a WebSocket connection to WSS.
  ...
  }.bind(this)); // Asynchronously join the room.
  ...
  var joinPromise = this.joinRoom_().then(function (roomParams) {
    ...
    return Promise.reject(error);
  }.bind(this)); // We only register with WSS if the web socket connection is open and if we're
  // already registered with GAE.

  Promise.all([channelPromise, joinPromise]).then(function () {
    this.channel_.register(this.params_.roomId, this.params_.clientId); // We only start signaling after we have registered the signaling channel
  ...
  if (shouldRequestIceServers) {
    ...
    iceServerPromise = requestIceServers(requestUrl, this.params_.iceServerTransports).then(function (iceServers) {
      ...
      if (this.onturnstatusmessage) {
        ...
        this.onturnstatusmessage('No TURN server; unlikely that media will traverse networks. ' + 'If this persists please ' + '<a href="mailto:discuss-webrtc@googlegroups.com?' + 'subject=' + subject + '">' + 'report it to discuss-webrtc@googlegroups.com</a>.');
      }
