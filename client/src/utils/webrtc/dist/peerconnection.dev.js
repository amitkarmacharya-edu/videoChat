...
var PeerConnectionClient = function PeerConnectionClient(params, startTime) {
  this.params_ = params;
  this.startTime_ = startTime;
  trace('Creating RTCPeerConnnection with:\n' + '  config: \'' + JSON.stringify(params.peerConnectionConfig) + '\';\n' + '  constraints: \'' + JSON.stringify(params.peerConnectionConstraints) + '\'.'); // Create an RTCPeerConnection via the polyfill (adapter.js).
  ...
  this.started_ = false; // TODO(jiayl): Replace callbacks with events.
...
PeerConnectionClient.prototype.receiveSignalingMessage = function (message) {
  ...
  if (this.isInitiator_ && messageObj.type === 'answer' || !this.isInitiator_ && messageObj.type === 'offer') {
    this.hasRemoteSdp_ = true; // Always process offer before candidates.
...
PeerConnectionClient.prototype.onSetRemoteDescriptionSuccess_ = function () {
  trace('Set remote session description success.'); // By now all onaddstream events for the setRemoteDescription have fired,
...
PeerConnectionClient.prototype.filterIceCandidate_ = function (candidateObj) {
  var candidateStr = candidateObj.candidate; // Always eat TCP candidates. Not needed in this context.
